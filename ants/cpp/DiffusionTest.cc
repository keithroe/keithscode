...
int main( int argc, char** argv )
{
    ...
    map( ant0 ).ant_id   = 0;
    map( ant1 ).ant_id   = 0;
    map.setPriority( Map::EXPLORE, hill, 100.0f );
    map.setPriority( Map::EXPLORE, ant0, -10.0f );
    map.setPriority( Map::EXPLORE, ant1, -10.0f );

    std::cout << map << std::endl;

    std::cerr << " 1 --------------" << std::endl;
    map.diffusePriority( Map::EXPLORE, 1 );
    std::cout << map << std::endl;
    std::cerr << " 2 --------------" << std::endl;
    map.diffusePriority( Map::EXPLORE, 1 );
    std::cout << map << std::endl;
    ...
    Timer timer;
    timer.start();
    map.diffusePriority( Map::EXPLORE, 50 );
    std::cout << map << std::endl;
